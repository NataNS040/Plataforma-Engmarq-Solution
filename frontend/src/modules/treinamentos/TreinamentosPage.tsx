...
function AddTreinamentoModal({ colab, tipos, empresaId, onClose }: AddTreinamentoModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (certificadoFile) certUrl = await uploadCertificado(certificadoFile, empresaId)
    ...
    >
      <div style={{ background: "var(--surface)", borderRadius: 16, padding: 24, width: 460, maxWidth: "95vw", boxShadow: "0 20px 60px rgba(0,0,0,0.18)" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-600)", display: "block", marginBottom: 6 }}>Certificado (opcional)</label>
            <div
              ...
              style={{
                border: `2px dashed ${isDragOver ? "var(--orange-500, #f97316)" : "var(--border)"}`,
                borderRadius: 8,
                padding: "14px 12px",
                textAlign: "center",
                cursor: "pointer",
                background: isDragOver ? "rgba(249,115,22,0.05)" : "var(--bg)",
                transition: "border-color 0.15s, background 0.15s",
              }}
            >
              {certificadoFile ? (
                <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "center" }}>
                  <FileText size={14} style={{ color: "var(--orange-600, #ea580c)", flexShrink: 0 }} />
                  <span style={{ fontSize: 12.5, fontWeight: 600, color: "var(--ink-800)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 280 }}>{certificadoFile.name}</span>
                  <button
                    type="button"
                    onClick={e => { e.stopPropagation(); setCertificadoFile(null); if (fileInputRef.current) fileInputRef.current.value = "" }}
                    style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "var(--ink-400)", display: "flex", alignItems: "center" }}
                  >
                    <X size={12} />
                  </button>
                </div>
                ...
                <div style={{ color: "var(--ink-400)", fontSize: 12.5 }}>
                  <UploadCloud size={18} style={{ margin: "0 auto 4px", display: "block", color: "var(--ink-300)" }} />
                  <span>Arraste o certificado ou <u style={{ cursor: "pointer" }}>clique para selecionar</u></span>
                </div>
              )}
            </div>
            <input
              ...
              style={{ display: "none" }}
              onChange={e => { const f = e.target.files?.[0]; if (f) setCertificadoFile(f) }}
            />
          </div>

          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 4 }}>
            <button type="button" className="tbtn ghost" onClick={onClose}>Cancelar</button>
            <button type="submit" className="tbtn primary" disabled={isSubmitting} style={{ justifyContent: "center" }}>
              {isSubmitting ? "Salvando…" : "Registrar treinamento"}
            </button>
          </div>
        </form>
      </div>
