...
export async function registrarTreinamento(input: TreinamentoInput): Promise<Treinamento> {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      certificado_url:     input.certificado_url ?? null,
    })
    .select('*')
    .single()
...
export async function uploadCertificado(file: File, empresaId: string): Promise<string> {
  ...
  const path = `${empresaId}/certificados/${crypto.randomUUID()}.${ext}`
  const { error } = await supabase.storage.from('documentos').upload(path, file)
  if (error) throw handleSupabaseError(error as never, 'Não foi possível fazer upload do certificado.')
  return path
}
